...
このファイルは、Expense-reporterプロジェクトの開発において守るべきルールと規約を定義します。
...
### ブランチ命名規則
- **feature/機能名** - 新機能開発（例: `feature/ocr-integration`, `feature/settings-page`）
- **bugfix/バグ名** - バグ修正（例: `bugfix/date-extraction-error`, `bugfix/spreadsheet-write-fail`）
...
- [ ] **feature/xxxブランチを作成**
  ...
  # 例: git checkout -b feature/add-ocr-api
  ```
...
- [ ] `CURRENT_STATUS.md`を更新
  ...
  - 「Git状態」を更新（ブランチ名含む）
- [ ] **関連ドキュメントを更新**（該当する場合）
  - テスト方法が変わった場合 → `tests/README.md`, `README.md`を更新
...
5. タグを作成（バージョン管理）
  ```bash
  git tag -a v0.1.0 -m "リリース v0.1.0: 〇〇機能追加"
...
### 例
```bash
git commit -m "機能: OpenAI APIによる日時抽出機能を追加"
git commit -m "修正: 議題選択時のクラッシュを修正"
git commit -m "改善: スプレッドシート書き込みのパフォーマンス改善"
git commit -m "ドキュメント: DEVELOPMENT_RULES.mdを更新"
git commit -m "WIP: 設定画面の実装途中"
```
...
export function extractDateTime(imageUrl: string): Promise<DateTimeResult> {
  // ...
}

// ❌ Bad
export function extractDateTime(imageUrl) {
  // ...
}
```

#### 命名規則
- **変数・関数**: camelCase (`userName`, `extractDateTime`)
- **型・インターフェース**: PascalCase (`MeetingMinute`, `TopicMaster`)
- **定数**: UPPER_SNAKE_CASE (`MAX_IMAGE_SIZE`, `SPREADSHEET_URL`)
...
```typescript
// 領収書画像から日時を抽出
export async function extractDateTime(imageUrl: string): Promise<DateTimeResult> {
  // ...
}
```
...
export default function MinuteCreatePage({ receiptImage, onComplete }: Props) {
  // ...
}
```
...
```typescript
// app/api/extract-datetime/route.ts
...
### Expense-reporterの設計哲学
1. **入力の最小化**: 自動抽出とテンプレート選択で手間を削減
2. **税務要件の確実性**: 必要事項を漏れなく記録
3. **シンプルなUX**: 直感的な操作フロー
4. **エラー防止**: バリデーションとフィードバック
